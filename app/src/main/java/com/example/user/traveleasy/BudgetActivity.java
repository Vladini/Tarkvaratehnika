...
public class BudgetActivity extends AppCompatActivity {
    ...
    public void saveChanges(View view) {
        DBHandler db = new DBHandler(this);
        Trip trip = new Trip(index, db.getTrip(index).getName(),db.getTrip(index).getStartDate(), db.getTrip(index).getEndDate(), Double.parseDouble(editText.getText().toString()), Double.parseDouble(editText2.getText().toString()), Double.parseDouble(editText3.getText().toString()) );
        db.updateTrip(trip);
